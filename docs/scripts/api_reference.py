    ...
    for header, section in sections:
        if header is None:
            ...
            signature = "{0}.{1}{2}".format(
                module.__name__, kls.__name__, str(inspect.signature(kls))
            )
            markdown.append(format_signature(signature))
            if len(sections) == 1 and any(c is nn.Module for c in inspect.getmro(kls)):
                markdown.append(
                    format_notes(["See: `torch.nn.{0}`".format(kls.__name__[4:])])
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--config",
        type=str,
        default="docs/scripts/config.yml",
