...
def model():
    ...
    model.weight.data = torch.tensor([[2.0, 3.0, 5.0]])
    return model
...
def test_update_parameters(model):
    ...
    """
    train_inputs = torch.tensor([[1.0, 2.0, 3.0]])
    train_loss = 0.5 * (model(train_inputs) ** 2)

    params = gradient_update_parameters(
        model, train_loss, params=None, step_size=0.5, first_order=False
    )

    assert train_loss.item() == 264.5
    assert list(params.keys()) == ["weight"]
    assert torch.all(params["weight"].data == torch.tensor([[-9.5, -20.0, -29.5]]))
    ...
    """
    test_inputs = torch.tensor([[4.0, 5.0, 6.0]])
    ...
    assert len(grads) == 1
    assert torch.all(grads[0].data == torch.tensor([[3780.0, 8505.0, 13230.0]]))
...
def test_update_parameters_first_order(model):
    ...
    """
    train_inputs = torch.tensor([[4.0, 5.0, 6.0]])
    train_loss = 0.5 * (model(train_inputs) ** 2)

    params = gradient_update_parameters(
        model, train_loss, params=None, step_size=0.5, first_order=True
    )

    assert train_loss.item() == 1404.5
    assert list(params.keys()) == ["weight"]
    assert torch.all(params["weight"].data == torch.tensor([[-104.0, -129.5, -154.0]]))
    ...
    assert len(grads) == 1
    assert torch.all(grads[0].data == torch.tensor([[-825.0, -1650.0, -2475.0]]))
...
def test_multiple_update_parameters(model):
    ...
    params_1 = gradient_update_parameters(
        model, train_loss_1, params=None, step_size=1.0, first_order=False
    )

    assert train_loss_1.item() == 264.5
    assert list(params_1.keys()) == ["weight"]
    assert torch.all(params_1["weight"].data == torch.tensor([[-21.0, -43.0, -64.0]]))
    ...
    params_2 = gradient_update_parameters(
        model, train_loss_2, params=params_1, step_size=1.0, first_order=False
    )

    assert train_loss_2.item() == 44700.5
    assert list(params_2.keys()) == ["weight"]
    assert torch.all(params_2["weight"].data == torch.tensor([[278.0, 555.0, 833.0]]))
    ...
    params_3 = gradient_update_parameters(
        model, train_loss_3, params=params_2, step_size=1.0, first_order=False
    )

    assert train_loss_3.item() == 7554384.5
    assert list(params_3.keys()) == ["weight"]
    assert torch.all(
        params_3["weight"].data == torch.tensor([[-3609.0, -7219.0, -10828.0]])
    )
    ...
    assert test_loss.item() == -115499.0
    assert len(grads) == 1
    assert torch.all(grads[0].data == torch.tensor([[-5020.0, -10043.0, -15066.0]]))
