...
def test_datasets_helpers_dataloader(name, shots, split):
    ...
    dataset = function(
        folder, ways=5, shots=shots, test_shots=15, meta_split=split, download=download
    )
...
def test_overflow_length_dataloader():
    ...
    dataset = helpers.omniglot(
        folder, ways=20, shots=1, test_shots=5, meta_train=True, download=download
    )
