...
def test_pairwise_similarity():
    ...
    similarities_th = pairwise_cosine_similarity(
        embeddings1_th, embeddings2_th, eps=eps
    )
    ...
    for i in range(2):
        for j in range(3):
            for k in range(5):
                for l in range(7):
                    similarities_np[i, j, k] += (
                        embeddings1_np[i, j, l] * embeddings2_np[i, k, l]
                    )
...
def test_pairwise_similarity_zero():
    ...
    embeddings1_np[0, 1] = 0.0  # Zero out one embedding
...
def test_matching_log_probas():
    ...
    log_probas_th = matching_log_probas(
        embeddings_th, targets_th, test_embeddings_th, num_classes, eps=eps
    )

    assert log_probas_th.shape == (2, 11, 5)
    np.testing.assert_array_less(log_probas_th.numpy(), 0.0)
...
def test_matching_probas():
    ...
    probas_th = matching_probas(
        embeddings_th, targets_th, test_embeddings_th, num_classes, eps=eps
    )

    assert probas_th.shape == (2, 11, 5)
    assert np.all(probas_th.numpy() >= 0.0)
    assert np.all(probas_th.numpy() <= 1.0)
    np.testing.assert_allclose(probas_th.sum(1).numpy(), 1.0, atol=1e-5)

    similarities_th = pairwise_cosine_similarity(
        embeddings_th, test_embeddings_th, eps=eps
    )
    ...
    for i in range(2):
        for j in range(3):
            l = targets_np[i, j]
            for k in range(5):
                probas_np[i, l, k] += exp_similarities_np[i, j, k] / np.sum(
                    exp_similarities_np[i, :, k]
                )
