...
def model():
    model = MetaSequential(
        MetaLinear(2, 3, bias=True), nn.ReLU(), MetaLinear(3, 1, bias=False)
    )
...
def params(prefix=""):
    weight_0 = torch.tensor(
        [[0.02, 0.03], [0.05, 0.07], [0.11, 0.13]], dtype=torch.float32
    )
...
def _dict_to_device(params, device):
    return dict((name, param.to(device=device)) for (name, param) in params.items())
