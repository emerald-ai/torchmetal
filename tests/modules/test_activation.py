...
def test_metamultiheadattention(bias, add_bias_kv, kdim, vdim):
    meta_model = MetaMultiheadAttention(
        3 * 5, 3, bias=bias, add_bias_kv=add_bias_kv, kdim=kdim, vdim=vdim
    )
    ...
    outputs_torchmetal, weights_torchmetal = meta_model(query, key, value, params=None)
    ...
    np.testing.assert_equal(
        weights_torchmetal.detach().numpy(), weights_nn.detach().numpy()
    )
...
def test_metamultiheadattention_params(bias, add_bias_kv, kdim, vdim):
    ...
    outputs_torchmetal, weights_torchmetal = meta_model(
        query, key, value, params=params
    )
    outputs_nn, weights_nn = model(query, key, value)
