...
class Rotation(object):
    def __init__(self, angle, **kwargs):
        ...
        if isinstance(angle, (list, tuple)):
            ...
            if angle % 360 == 0:
                import warnings
                warnings.warn('Applying a rotation of {0} degrees (`{1}`) '
                              'as a class augmentation on a dataset is '
                              'equivalent to the original dataset.'.format(
                                  angle, self), UserWarning, stacklevel=2)
        self.kwargs = kwargs

    def __iter__(self):
        return iter(Rotation(angle, **self.kwargs) for angle in self._angles)
    ...
    def __call__(self, image):
        ...
        return F.rotate(image, self.angle % 360, **self.kwargs, fill=fill)
