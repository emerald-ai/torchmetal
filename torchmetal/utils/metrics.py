...
from torchmetal.utils.prototype import get_prototypes
...
def _pad_images(inputs, size=(224, 224), **kwargs):
    ...
    padding = (
        pad_width,
        size[1] - width - pad_width,
        pad_height,
        size[0] - height - pad_height,
    )
...
def hardness_metric(batch, num_classes):
    ...
    if ("train" not in batch) and ("support" not in batch):
        raise ValueError(
            "The tasks do not contain any training/support set. "
            'Make sure the tasks contain either the "train" or the '
            '"support" key.'
        )
    if ("test" not in batch) and ("query" not in batch):
        raise ValueError(
            "The tasks do not contain any test/query set. Make "
    ...
    with torch.no_grad():
        ...
        backbone = torch.hub.load(
            "pytorch/vision:v0.5.0", "resnet152", pretrained=True, verbose=False
        )
        backbone.eval()
        ...
        padded_train_inputs = _pad_images(
            train_inputs, size=(224, 224), mode="constant", value=0.0
        )
        padded_test_inputs = _pad_images(
            test_inputs, size=(224, 224), mode="constant", value=0.0
        )
