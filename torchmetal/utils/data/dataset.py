...
class CombinationMetaDataset(MetaDataset):
    ...
    def __getitem__(self, index, new_split=None):
        if isinstance(index, int):
            raise ValueError(
                "The index of a `CombinationMetaDataset` must be "
                "a tuple of integers, and not an integer. For example, call "
        ...
        if self.dataset_transform is not None:
            if new_split is not None:
                if not isinstance(self.dataset_transform, ClassSplitter):
                    raise TypeError(f'Object of type ClassSplitter expected, \
                                    however type \
                                    {type(self.dataset_transform)} \
                                    is member')
                self.dataset_transform.set_num_train_test(
                    num_samples_per_class=new_split["num_samples"],
                    num_train_per_class=new_split["num_train"],
                    num_test_per_class=new_split["num_test"],
                )
