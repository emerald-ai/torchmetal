...
class NonEpisodicWrapper(Dataset):
    ...
    def __init__(self, dataset, target_transform=None):
        ...
        if not isinstance(dataset, CombinationMetaDataset):
            raise ValueError(
                "`NonEpisodicWrapper` can only be wrapped around a "
    ...
    def __getitem__(self, index):
        ...
        class_index = np.maximum(
            np.searchsorted(self._offsets, index % self.num_samples, side="left") - 1, 0
        )
        ...
        image = (
            Image.open(io.BytesIO(array)) if array.ndim < 2 else Image.fromarray(array)
        )

        class_augmented_index = (
            class_dataset.num_classes * (index // self.num_samples) + class_index
        )
        transform = class_dataset.get_transform(
            class_augmented_index, class_dataset.transform
        )
