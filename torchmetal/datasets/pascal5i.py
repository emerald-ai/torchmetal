...
class Pascal5iClassDataset(ClassDataset):
    ...
    def __init__(
        ...
        super(Pascal5iClassDataset, self).__init__(
            meta_train=meta_train,
            meta_val=False,
            meta_test=meta_test,
            meta_split=meta_split,
    ...
    def load_dict_per_class(self):
        ...
        for k, v in new_exist_class_list.items():
            ...
            for path in v:
                fname = os.path.join(
                    self.root, "VOCdevkit/VOC2012/JPEGImages", path + ".jpg"
                )
                if cname not in images:
                    images[cname] = []
                images[cname].append(fname)
                fname = os.path.join(
                    self.root, "Binary_map_aug", self.meta_split, str(k), path + ".png"
                )
    ...
    def read_labels(self, fold=None):
        ...
        if fold is not None:
            if self.meta_train:
                in_classes = set(range(21)) - set(
                    range(fold * 5 + 1, (fold + 1) * 5 + 1)
                )
            else:
...
class PascalDataset(Dataset):
    def __init__(self, index, data, class_id, transform=None, target_transform=None):
