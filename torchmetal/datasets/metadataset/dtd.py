...
class Dtd(CombinationMetaDataset):
    """
    Parameters
    ----------
    root : string
        Root directory where the dataset folder `dtd` exists.
    ...
    convert : bool (default: `False`)
        ...
        directory (under the `dtd` folder). If the dataset is already
...
class DtdClassDataset(ClassDataset):
    def __init__(
        self,
        root,
        meta_dataset=False,
        ...
        super(DtdClassDataset, self).__init__(
            meta_dataset=meta_dataset,
            meta_train=meta_train,
            meta_val=meta_val,
            meta_test=meta_test,
        ...
        if not self._check_integrity():
            raise RuntimeError("dtd integrity check failed")
