...
from torchmetal.datasets.metadataset.data import dataset_spec as ds_spec
from torchmetal.datasets.metadataset.data import imagenet_specification
...
VGGFLOWER_LABELS_PATH = os.path.join(AUX_DATA_PATH, 'VggFlower_labels.txt')
...
def write_from_npy_single_channel(class_npy_file, class_label,
    ...
    Args:
        ...
        output_path: the location to write the hdf5 file.
        Returns:
...
def write_to_single_label_dataset(collection,
                                  class_label,
                                  writer):
    """Adds image collection to h5df dataset array of one label to images
    structure
    Args:
        collection: collection of image data bytes
        class_label: label for images
        writer: hdf5 file writer
    """
    collection = np.asarray(collection)
    writer.create_dataset(f"{class_label}", data=collection)
...
def write_from_image_files(class_files,
                           ...
                           output_format='JPEG',
                           skip_on_error=False):
    """Create and write a hdf5 file for the images corresponding to a group of
    labels.
    ...
    Args:
        ...
        image data is encoded inside the file.
    ...
    def load_and_process_image(path, bbox=None):
        ...
        If that is not the case, the resulting image is encoded to
        output_format.
    ...
    for i, path in enumerate(class_files):
        ...
        try:
            ...
            if skip_on_error:
                logging.warn('While trying to load file %s, got error: %s',
                                path, e)
            else:
                raise
        else:
...
def write_from_directory(class_directory,
                                  class_label,
                                  output_path,
                                  writer,
                                  invert_img=False,
    ...
    written_images_count = write_from_image_files(
        class_files,
        class_label,
        output_path,
        writer,
...
class DatasetConverter(object):
  ...
  def __init__(self,
    ...
    Path.mkdir(Path(self.records_root), parents=True, exist_ok=True)
    ...
    if self.split_file is None:
      ...
      shutil.copy(stored_split, self.split_file)
        ...
        self.split_file = os.path.join(self.splits_root,
                                        '{}_splits.json'.format(self.name))
        lib_splits_root = os.path.join(os.path.dirname(__file__), "splits")
        lib_split_file = os.path.join(self.splits_root,
                                        '{}_splits.json'.format(self.name))
        shutil.copy(lib_split_file, self.split_file)
  ...
  def create_dataset_specification_and_records(self):
    ...
    The reader data sources operate under this assumption.  Secondly, a tf.record needs to be created and written for each class. There
...
class OmniglotConverter(DatasetConverter):
  ...
  def parse_split_data(self, split, alphabets, alphabets_path, writer):
    ...
    for alphabet_folder_name in alphabets:
      ...
      for char_folder_name in sorted(os.listdir(alphabet_path)):
        ...
        write_from_directory(
            class_path, class_label, class_records_path, writer,
            invert_img=True)
  ...
  def create_dataset_specification_and_records(self):
    ...
    with h5py.File(f'{self.records_path}/data.h5', 'w') as writer:
        self.parse_split_data(learning_spec.Split.TRAIN, training_alphabets,
                            data_path_trainval, writer)
...
class QuickdrawConverter(DatasetConverter):
  ...
  def parse_split_data(self, split, split_class_names, writer):
    """Parse the data of the given split.
    Specifically, update self.class_names, self.images_per_class, and
    self.classes_per_split with the information for the given split, and
    create and write records of the classes of the given split.
  ...
  def create_dataset_specification_and_records(self):
    ...
    with h5py.File(f'{self.records_path}/data.h5', 'w') as writer:
        self.parse_split_data(learning_spec.Split.TRAIN, train_classes, writer)
        self.parse_split_data(learning_spec.Split.VALID, valid_classes, writer)
        self.parse_split_data(learning_spec.Split.TEST, test_classes, writer)
...
class CUBirdsConverter(DatasetConverter):
  ...
  def create_splits(self):
    ...
    with open(os.path.join(self.data_root, 'classes.txt'),
                           'r') as f:
      class_names = []
      for lines in f:
  ...
  def create_dataset_specification_and_records(self):
    ...
    with h5py.File(f'{self.records_path}/data.h5', 'w') as writer:
        for class_id, class_label in enumerate(all_classes):
            ...
            write_from_directory(class_directory, class_id,
                                            class_records_path, writer)
  ...
  def create_dataset_specification_and_records(self):
    ...
    with h5py.File(f'{self.records_path}/data.h5', 'w') as writer:
        for class_id, class_label in enumerate(all_classes):
            ...
            # Create and write the tf.Record of the examples of this class.
            write_from_image_files(class_paths, class_id, class_records_path, writer)
  ...
  def create_dataset_specification_and_records(self):
    ...
    with h5py.File(f'{self.records_path}/data.h5', 'w') as writer:
        for class_id, class_name in enumerate(all_classes):
            ...
            self.images_per_class[class_id] = write_from_directory(
                class_directory,
                class_id,
                class_records_path,
                writer,
  ...
  def create_dataset_specification_and_records(self):
    ...
    with h5py.File(f'{self.records_path}/data.h5', 'w') as writer:
        for class_id, class_name in enumerate(all_classes):
            ...
            write_from_image_files(
                class_files, class_id, class_records_path, writer,
                bboxes=bboxes)
...
class MSCOCOConverter(DatasetConverter):
  ...
  def create_dataset_specification_and_records(self):
    ...
    for i, annotation in enumerate(self.coco_instance_annotations):
      ...
      logging.info('appending image %d/%d', i,
      ...
      collection[class_id].append(image_crop_bytes.getvalue())
      self.images_per_class[class_id] += 1

    class_label = 0
    with h5py.File(f'{self.records_path}/data.h5', 'w') as writer:
        write_to_single_label_dataset(collection[class_label], class_label, writer)
        class_label += 1
...
class ImageNetConverter(DatasetConverter):
  ...
  def _create_data_spec(self, train_split_only=False):
    ...
    if not self.ilsvrc_2012_num_leaf_images_path:
      self.ilsvrc_2012_num_leaf_images_path = os.path.join(self.records_path,
                                                      'num_leaf_images.json')
    specification = imagenet_specification.create_imagenet_specification(
        learning_spec.Split,
        self.files_to_skip,
        self.ilsvrc_2012_num_leaf_images_path,
    ...
    with h5py.File(f'{self.records_path}/data.h5', 'w') as writer:
        for classes, split in zip([train_classes, valid_classes, test_classes],
            ...
            for class_id, class_name in zip(classes, names):
                ...
                write_to_single_label_dataset(collection, class_id, writer)
